<template>
    ...
    <div class="thumbnail-container">
        <img v-if="form.data && form.data.strDrinkThumb" :src="form.data.strDrinkThumb" alt="a1 Thumbnail" loading="lazy" />
    </div>
...
import { useCocktailStore } from '../stores/store';
  ...
  onMounted(async () => {
  await cocktailStore.fetchCocktails('a1');
...
const ingredientMeasures = computed(() => {
  ...
  watch(() => form.value.data, (newData) => {
    console.log('Form data updated a1:', newData);
  });  
